...
type Client struct {
	...
	opt      *Option
	sending  sync.Mutex       // protect following 互斥锁，为了保证请求的有序发送
	header   codec.Header     // 每个请求的消息头
	mu       sync.Mutex       // protect following
	seq      uint64           // 用于给发送的请求编号，每个请求拥有唯一编号
	pending  map[uint64]*Call // 存储未处理完的请求
	closing  bool             // user has called Close 用户主动关闭，即调用Close方法
...
// 服务器或客户端发生错误时调用，将 shutdown 设置为 true，且将错误信息通知所有 pending 状态的 call
...
func dialTimeout(f newClientFunc, network, address string, opts ...*Option) (client *Client, err error) {
	...
	select {
	case <-time.After(opt.ConnectTimeout): // 如果 time.After() 先接收到消息，则说明 NewClient 执行超时，返回错误
