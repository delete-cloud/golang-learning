...
func (xc *XClient) dial(rpcAddr string) (*Client, error) {
	...
	if ok && !client.IsAvailable() {  // 如果有，检查是否为可用状态，如果是则返回缓存的 Client
		_ = client.Close()
		delete(xc.clients, rpcAddr) // 如果不可用，则从缓存中删除
		client = nil
	}
	if client == nil { // 如果没有返回缓存的 Client，则说明需要创建新的 Client，缓存并返回
		var err error
...
func (xc *XClient) Broadcast(ctx context.Context, serviceMethod string, args, reply interface{}) error {
	...
	var mu sync.Mutex // protect e and replyDone 并发情况下使用互斥锁保证 error 与 reply 能被正确赋值
	var e error
	replyDone := reply == nil              // if reply is nil, don't need to set value
	ctx, cancel := context.WithCancel(ctx) // 借助 context.WithCancel 确保有错误发生时，快速失败
	for _, rpcAddr := range servers {
		wg.Add(1) // 为了提升性能，请求是并发的
		go func(rpcAddr string) {
			...
			if err != nil && e == nil {
				e = err
				cancel() // if any call failed, cancel
			}
